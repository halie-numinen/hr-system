        ...
        with open('./employee.data') as datafile:
            ...
            for row in reader:
                ...
                if employee_type == "Executive":
                    employee = Executive(row[1], row[2], float(row[4]), Role(int(row[5])))
                    employee.image = row[3]
                if employee_type == "Manager":
                    employee = Manager(row[1], row[2], float(row[4]), Department(int(row[5])))
                    employee.image = row[3]
                if employee_type == "Permanent":
                    employee = Permanent(row[1], row[2], float(row[4]), row[5])
                    employee.image = row[3]
                if employee_type == "Temp":
                    employee = Temporary(row[1], row[2], float(row[4]), row[5])
                    employee.image = row[3]
...
class SalariedForm(EmployeeForm):
    ...
    def fill_in(self, index) -> None:
        super().fill_in(index)
        self._salary_edit.setText(str(self._employee.yearly))
        self.layout.addRow(QLabel("Salary: "), self._salary_edit)

    def update_employee(self) -> None:
        try:
            super().update_employee()
            self._employee.yearly = float(self._salary_edit.text())
            print("1")
...
class ExecutiveForm(SalariedForm):
    def __init__(self, parent=None):
        ...
        self._box = QComboBox()
        for role in Role:
            self._box.addItem(str(role))
        self._role_edit = QLineEdit()

    def fill_in(self, index) -> None:
        super().fill_in(index)
        role_option = self._employee.role
        self._box.setCurrentText(str(role_option))
        self.layout.addRow(QLabel("Role: "), self._box)

    def update_employee(self) -> None:
        try:
            super().update_employee()
            self._employee.role = self._box.currentText()
            print("2")
        except Exception as ex:
            print(ex)
...
class ManagerForm(SalariedForm):
    def __init__(self, parent=None):
        super().__init__(parent)
        self._box2 = QComboBox()
        for department in Department:
            self._box2.addItem(str(department))
        self.layout.addRow(QLabel("Head of Department: "), self._box2)

    def fill_in(self, index) -> None:
        super().fill_in(index)
        dep = self._employee.department
        self._box2.setCurrentText(str(dep))

    def update_employee(self) -> None:
        try:
            self._employee.department = Department[self._box2.currentText().upper()]
            super().update_employee()
            ManagerForm.save_file()
            print("3")
        except Exception as ex:
            print(ex)
...
class HourlyForm(EmployeeForm):
    def __init__(self, parent):
        super().__init__(parent)
        self._hourly_edit = QLineEdit()

    def fill_in(self, index) -> None:
        super().fill_in(index)
        self._hourly_edit.setText(str(self._employee.hourly))
        self.layout.addRow(QLabel("Hourly Wage: "), self._hourly_edit)

    def update_employee(self) -> None:
        super().update_employee()
        self._employee.hourly = float(self._hourly_edit.text())
...
class PermanentForm(HourlyForm):
    def __init__(self, parent=None):
        super().__init__(parent)
        self._hired_date = QLabel()
        self.layout.addRow(QLabel('Hired date:'), self._hired_date)

    def fill_in(self, index) -> None:
        super().fill_in(index)
        self._hired_date.setText(str(self._employee.hired_date))
